...
def findBoxes(preprocessedImage, textClr, conf, minw, maxw, minh, maxh, configStr, strokeWidth):
    buttonImageBoxes = pytesseract.image_to_data(preprocessedImage, config=configStr, lang='eng', output_type=pytesseract.Output.DICT)

    print(buttonImageBoxes['text'])

    for i in range(len(buttonImageBoxes['text'])):
        if int(buttonImageBoxes['conf'][i]) > conf:
            (x, y, w, h) = (buttonImageBoxes['left'][i], buttonImageBoxes['top'][i], buttonImageBoxes['width'][i], buttonImageBoxes['height'][i])
            if minw < w and w < maxw and minh < h and h < maxh: 
                ...
                cv2.rectangle(preprocessedImage, pt1 = (x, y), pt2 = (x+w, y+h), color = textClr, thickness = strokeWidth)
                cv2.putText(img=preprocessedImage, text=label, org=(x + 5, y+h-10), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=1, color=textClr,thickness=1)
                print(label + " :: " + str(buttonImageBoxes['conf'][i]) + " -- " + str(x) + ":" + str(y) + ":" + str(w) + ":" + str(h))
...
def buttonOCR(buttonMat):
    ...
    (T, preprocessedImage) = cv2.threshold(buttonMat, 70, 255, cv2.THRESH_BINARY)
    preprocessedImage = cv2.bitwise_not(preprocessedImage)
    return findBoxes(preprocessedImage, (0, 0, 255), 10, 1, 500, 1, 500, "--psm 9", 5)

# buttonImage = cv2.imread(r'./elevator_panels/val/elevatorbuttons.jpg')
